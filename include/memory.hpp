...
struct VmaBuffer {
  ...
  ~VmaBuffer() { if (buffer)vmaDestroyBuffer(allocator, buffer, allocation); }
};
...
struct VmaImage {
  ...
  vk::Image image;
  VmaAllocation allocation;
  ~VmaImage() { if (image) vmaDestroyImage(allocator, image, allocation); }
};

struct Alloc_State {
  VmaAllocator allocator;

  static Alloc_State create(vk::Device device, vk::PhysicalDevice phdevice) {
