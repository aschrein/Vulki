...
void main() {
    ...
    ivec2 dim = imageSize(out_image[nonuniformEXT(push_constants.dst_level)]);

    if (gl_GlobalInvocationID.x > dim.x || gl_GlobalInvocationID.y > dim.y)
      return;
    ivec2 xy = ivec2(gl_GlobalInvocationID.xy);
    #define src in_image[nonuniformEXT(push_constants.src_level)]
    if (push_constants.copy != 0) {
      vec4 in_value = texelFetch(src, xy, 0);
      imageStore(out_image[nonuniformEXT(push_constants.dst_level)],
                          xy,
                          in_value
                          );
    } else {
