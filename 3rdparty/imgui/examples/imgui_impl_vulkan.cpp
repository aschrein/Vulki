...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (!g_DescriptorPool)
    {
        ...
        err = vkCreateDescriptorPool(v->Device, &pool_info, 0, &g_DescriptorPool);
        check_vk_result(err);
    }
...
void ImGui_ImplVulkan_NewFrame()
{
    ImGui_ImplVulkan_InitInfo* v = &g_VulkanInitInfo;
    ImGuiIO& io = ImGui::GetIO();
    //VkResult err = vkResetDescriptorPool(v->Device, g_DescriptorPool, 0);
//    check_vk_result(err);
    // recreate font descriptor set
    if (!io.Fonts->TexID) {
      VkDescriptorSet font_descriptor_set = (VkDescriptorSet)ImGui_ImplVulkan_AddTexture(g_FontSampler, g_FontView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
      io.Fonts->TexID = (ImTextureID)font_descriptor_set;
    }
...
ImTextureID ImGui_ImplVulkan_AddTexture(VkSampler sampler,
