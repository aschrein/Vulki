...
TEST(graphics, vulkan_graphics_shader_test_3) {
  auto device_wrapper = init_device(true);
  auto &device = device_wrapper.device;
  vk::DynamicState dynamic_states[] = {
      vk::DynamicState::eViewport,
  ...
  auto ubo_buffer = alloc_state->allocate_buffer(
      vk::BufferCreateInfo()
          ...
          .setUsage(vk::BufferUsageFlagBits::eUniformBuffer |
                    vk::BufferUsageFlagBits::eTransferDst),
      VMA_MEMORY_USAGE_CPU_TO_GPU);
  uint N_PARTICLES = 10000;
  uint GRID_DIM = 16;
  auto bins_buffer = alloc_state->allocate_buffer(
      vk::BufferCreateInfo()
          .setSize(sizeof(uint) * GRID_DIM * GRID_DIM * GRID_DIM * 2)
          .setUsage(vk::BufferUsageFlagBits::eStorageBuffer |
                    vk::BufferUsageFlagBits::eTransferDst),
      VMA_MEMORY_USAGE_CPU_TO_GPU);
  auto particles_buffer = alloc_state->allocate_buffer(
      vk::BufferCreateInfo()
          .setSize(sizeof(float) * N_PARTICLES * 3 * 5)
          .setUsage(vk::BufferUsageFlagBits::eStorageBuffer |
                    vk::BufferUsageFlagBits::eTransferDst),
      VMA_MEMORY_USAGE_CPU_TO_GPU);
  // Shared sampler
  ...
  for (u32 i = 0; i < N_PARTICLES; i++) {
    particles.push_back(frand.rand_unit_cube());
    ug.put(particles[i], 0.0f, i);
  }
  // Render offscreen
  device_wrapper.pre_tick = [&](vk::CommandBuffer &cmd) {
    if (framebuffer_wrapper.width != example_viewport.extent.width ||
    ...
    compute_pipeline_wrapped.update_descriptor(
        ...
        sizeof(uint) * GRID_DIM * GRID_DIM * GRID_DIM * 2,
        vk::DescriptorType::eStorageBuffer);
    compute_pipeline_wrapped.update_descriptor(
        device.get(), "Particles", particles_buffer.buffer, 0,
        sizeof(float) * N_PARTICLES * 3, vk::DescriptorType::eStorageBuffer);
    compute_pipeline_wrapped.update_storage_image_descriptor(
        device.get(), "resultImage", storage_image_wrapper.image_view.get());
    {
      void *data = ubo_buffer.map();
      ...
      tmp_ubo.ug_bin_size = 2.0f / GRID_DIM;
      *typed_data = tmp_ubo;
      ubo_buffer.unmap();
    }
    {
      ...
      }
      {
        ...
        for (u32 pid : packed.ids) {
          particles_packed.push_back(particles[pid]);
        }
        memcpy(typed_data, &particles_packed[0], particles_packed.size() * sizeof(vec3));
